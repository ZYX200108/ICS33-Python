...
class C:
    ...
    def __setattr__(self,name,value):
        ...
        if name.startswith('private_'):
            raise NameError
        if calling.function == '__init__' and C.in_C(calling):
            self.__dict__['private_{}'.format(name)] = value
        elif ('private_{}'.format(name)) in self.__dict__:
            if C.in_C(calling):
                self.__dict__['private_{}'.format(name)] = value
            else:
                raise NameError
    ...
    def __getattr__(self,name):
        ...
        if C.in_C(calling):
            if name in self.__dict__:
                ...
                return self.__dict__['private_{}'.format(name)]
            else:
                raise NameError
        else:
            if name in self.__dict__:
